...
interface NotesStore {
  ...
  refreshCurrentUserNotes: () => Promise<void> // New helper
...
export const useNotesStore = create<NotesStore>((set, get) => ({
  ...
  addNote: async (noteData: AddNoteData) => {
    ...
    const tempNote: Note = {
      ...
      preview: noteData.content.substring(0, 100) + (noteData.content.length > 100 ? '...' : ''),
      createdAt: now,
      updatedAt: now,
  ...
  togglePin: async (id) => {
    ...
    if (note) {
      console.log('📌 Toggling pin for note:', id, 'from', note.pinned, 'to', !note.pinned)
  ...
  toggleStar: async (id) => {
    const note = get().notes.find(n => n.id === id)
    if (note) {
      console.log('⭐ Toggling star for note:', id, 'from', note.starred, 'to', !note.starred)
